...
def bbx_extractor(results, frame_width, frame_height):
    xmin = int((results.multi_face_landmarks[0].landmark[234].x - 0.01)*frame_width)
    ymin = int((results.multi_face_landmarks[0].landmark[10].y - 0.01)*frame_height)
    
    xmax = int((results.multi_face_landmarks[0].landmark[454].x + 0.01)*frame_width)
    ymax = int((results.multi_face_landmarks[0].landmark[152].y + 0.01)*frame_height)
...
fps_list = deque(maxlen = 30) # Store the last 20 FPS values
frame_counter = 0 # Keep track of the number of frames processed

cv2.namedWindow('ComfortSense Protoype', cv2.WINDOW_NORMAL)
cv2.resizeWindow('ComfortSense Protoype', 1920 , 1080)
...
while cap.isOpened():
    ...
    cv2.imshow('ComfortSense Protoype', frame)
